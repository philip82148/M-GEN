...
async function generateMentions() {
    ...
    if(membersCsv) {
        ...
        $('.selected-file').html(`インストールフォルダ内:<b>${defaultMembersCsvName}</b>`);
    }

    searchAndShowResult(await fetchPageSource(), await csvToTrimmedArray(membersCsv));
}
    ...
    return new Promise((resolve, reject) => {
        ...
        for(const line of lines) {
            ...
            row.forEach(function(cell) {
                const trimmedCell = cell.replace(/^"|"$/g, '').replace(/""/, '"').trim();
                if(trimmedCell)
                    trimmedRow.push(trimmedCell);
            });
...
function searchAndShowResult(pageSource, memberList) {
    ...
    for(const member of memberList) {
        ...
        for(const searchTerm of member) {
            ...
            // 空白は空白を任意の数含む文字として検索
            // 途中のreplaceはエスケープ
            const searchRegExp = searchTerm.replace(/[.*+\-?^${}()|[\]\\]/g, '\\$&').replace(/[ 　]+/g, '( |　|&nbsp;)*');
    ...
    const unAtAndConvertMentionForComparison = function(originalMention) {
        ...
        return originalMention.replace(/^[@＠]/, '').replace(/　/g, ' ').toLowerCase().replace(/[ァ-ン]/g, function(s) {
...
/*    const shortenMention = function(originalMention) {
    ...
    const convertMention = function(originalMention, shorten) {
		...
		// 短くしないか、もともと1文字
		if(!shorten || unAttedMention.length <= 1)
			return '@' + unAttedMention;

		const convertedMention = unAtAndConvertMentionForComparison(originalMention);
        ...
        for(const member of resultMemberList) {
            const convertedLineName = unAtAndConvertMentionForComparison(member['lineName']);
			// それ自身は無視
			if(convertedLineName === convertedMention) continue;
            ...
            for(let index = 0; index < convertedMention.length; index++) {
				...
				// このインデックスに見込みがない
				if(currentLength >= convertedMention.length)
					continue;
				
				// それぞれのindexから始まるshortenedMentionを更新する
				while(convertedLineName.indexOf(indexesToShortenedMention[index]) !== -1) {
                    // 次の空白でない文字を探す
                    const additionalLength = convertedMention.substr(index + currentLength).search(/[^ ]/g);
					...
					indexesToShortenedMention[index] = convertedMention.substr(index, currentLength);
				}
			...
			if(minLength >= convertedMention.length)
				return '@' + unAttedMention;
        }
    ...
    // 結果表示
    const explanationIfChecked = 'コピペして左から(候補が出るのを待って)Enterで確定させてください。';
    $message.html(
            ...
            $p.click(function() {
                ...
                navigator.clipboard.writeText(mention).then(function() {
                    ...
                    if(mention.length <= 9) {
                        ...
                        firstMention = mention.substr(0, 9).replace(/@$/, '').trim() + '...';
                    }
    ...
    $message.find('input').on('change', function() {
        ...
        $message.find('.explanation').html($(this).prop('checked') ? explanationIfChecked : '');
    });
    ...
    $('.not-submitted').text(notSubmitted + '(' + String(notSubmittedCount) + '人)');
